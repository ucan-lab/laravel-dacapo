...
class IntegerType implements ColumnType
{
    ...
    public function createMigrationMethod(ColumnName $columnName): string
    {
        if ($this->autoIncrement && $this->unsigned) {
            return sprintf("->integer('%s', true, true)", $columnName->getName());
        } elseif ($this->autoIncrement && $this->unsigned === false) {
            return sprintf("->integer('%s', true)", $columnName->getName());
        } elseif ($this->autoIncrement === false && $this->unsigned) {
            return sprintf("->integer('%s', false, true)", $columnName->getName());
        }

        return sprintf("->integer('%s')", $columnName->getName());
    }
}
