...
class DoubleType implements ColumnType
{
    /**
    ...
    public function createMigrationMethod(ColumnName $columnName): string
    {
        ...
        if (is_array($args) && count($args) === 2) {
            return sprintf("->double('%s', %d, %d)", $columnName->getName(), $args[0], $args[1]);
        } elseif (is_array($args) && count($args) === 1) {
            return sprintf("->double('%s', %d)", $columnName->getName(), $args[0]);
        } elseif (is_int($args)) {
            return sprintf("->double('%s', %s)", $columnName->getName(), $args);
        } elseif (is_string($args)) {
            return sprintf("->double('%s', %s)", $columnName->getName(), $args);
        }

        return sprintf("->double('%s')", $columnName->getName());
    }
}
