...
class SmallIntegerType implements ColumnType
{
    ...
    public function createMigrationMethod(ColumnName $columnName): string
    {
        if ($this->autoIncrement && $this->unsigned) {
            return sprintf("->smallInteger('%s', true, true)", $columnName->getName());
        } elseif ($this->autoIncrement && $this->unsigned === false) {
            return sprintf("->smallInteger('%s', true)", $columnName->getName());
        } elseif ($this->autoIncrement === false && $this->unsigned) {
            return sprintf("->smallInteger('%s', false, true)", $columnName->getName());
        }
